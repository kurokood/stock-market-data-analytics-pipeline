...
# DynamoDB Table Name and SNS ARN from environment (with safe default for table name)
TABLE_NAME = os.environ.get("DYNAMODB_TABLE_NAME", "stock-market-data")
...
def get_recent_stock_data(symbol, minutes=5):
    ...
    try:
        response = table.query(
            KeyConditionExpression=Key("symbol").eq(symbol) & Key("timestamp").gte(past_time.strftime("%Y-%m-%dT%H:%M:%SZ")),
            ScanIndexForward=True  # Ascending order; we'll sort explicitly below just in case
        )
            ...
            if message:
                try:
                    if SNS_TOPIC_ARN:
                        ...
                        print("SNS_TOPIC_ARN not set in environment; skipping SNS publish")
                except Exception as e:
